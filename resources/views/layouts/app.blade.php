...
<head>
    ...
    <title>@yield('title', 'Desa Cantik - Data & Informasi Kampung Tanjung Perangat')</title>
    ...
    <div class="flex items-center gap-1">
        <a href="{{ url('/') }}"
           class="px-4 py-2 text-xs font-medium rounded transition-colors
                  {{ ($activeNav ?? '') === 'beranda' ? 'bg-green-100 text-green-700 font-bold' : 'text-gray-600 hover:text-green-700 hover:bg-gray-50' }}">
            Beranda
        </a>
        {{-- Statistik Dropdown --}}
        <div class="relative" id="statistik-dropdown-wrapper">
            <button
                ...
                class="flex items-center gap-1 px-4 py-2 text-xs font-medium rounded transition-colors
                       {{ ($activeNav ?? '') === 'statistik' ? 'bg-green-100 text-green-700 font-bold' : 'text-gray-600 hover:text-green-700 hover:bg-gray-50' }}">
                Statistik
            ...
            >
                ...
                {{-- Sarana & Prasarana --}}
                <a href="{{ url('/statistik/sarana-prasarana') }}"
                   class="flex items-center gap-3 px-4 py-2.5 hover:bg-green-50 hover:text-green-700 transition-colors group">
        ...
        <a href="{{ url('/spasial') }}"
           class="px-4 py-2 text-xs font-medium rounded transition-colors
                  {{ ($activeNav ?? '') === 'spasial' ? 'bg-green-100 text-green-700 font-bold' : 'text-gray-600 hover:text-green-700 hover:bg-gray-50' }}">
            Spasial
        </a>
        <a href="{{ url('/infografis') }}"
           class="px-4 py-2 text-xs font-medium rounded transition-colors
                  {{ ($activeNav ?? '') === 'infografis' ? 'bg-green-100 text-green-700 font-bold' : 'text-gray-600 hover:text-green-700 hover:bg-gray-50' }}">
            Infografis
        </a>
        <a href="{{ url('/publikasi') }}"
           class="px-4 py-2 text-xs font-medium rounded transition-colors
                  {{ ($activeNav ?? '') === 'publikasi' ? 'bg-green-100 text-green-700 font-bold' : 'text-gray-600 hover:text-green-700 hover:bg-gray-50' }}">
            Publikasi
        </a>
    </div>

    {{-- Login Button --}}
    <a href="{{ route('login') }}" id="btn-login" class="px-5 py-2 bg-green-900 text-white text-xs font-bold rounded-lg hover:bg-green-800 transition-colors flex-shrink-0">
        Login
    </a>
</nav>
